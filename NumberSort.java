...
public class NumberSort {
    ...
    /**
     * Two 2D arrays used for sorting - Each row represents a digit (0-9) - Each
     * column holds numbers assigned to that digit during sorting.
     *
     * We use Integer (object) instead of int (primitive) so we can set unused
     * elements to null. This avoids confusion in cases where the input array
     * might contain actual zero values that is needed to sort.
     */
    ...
    /**
     * Initializes two 2D arrays (array1 and array2) with 10 rows for digits 0–9
     * and 'size' columns (based on the number of elements to sort).
     *
     * Worst case: all numbers could go into the same digit row , so each row
     * needs 'size' slots to avoid overflow.
     *
     * Example: for input {23, 45, 12, 99, 5} (5 numbers) → creates array1 and
     * array2 of size [10][5].
     *
    ...
    /**
     * Reset the count of elements in each digit row to zero before the next
     * sorting pass.
     */
     ...
     *
     * @param array the 2D array to be cleared, (Setting all elements in the
     * digit row from 0-9 to null).
     */
    ...
    /**
     * Distributes numbers into the appropriate digit row based on the current
     * digit place value.
     *
     * This method handles the core step of radix sort, where numbers are sorted
     * by their digit values (0-9) at a given place value (1, 10, 100, 1000
     * etc.)
     *
     * Depending on whether we're sorting on the first pass (placeValue == 1),
     * or alternating between array1 and array2 on subsequent passes, numbers
     * are distributed into the correct digit row inside a 2D array.
     *
     * On the first pass (placeValue == 1), numbers from the input array are
     * placed in array1. On odd passes (log10(placeValue) % 2 == 1), numbers
     * from array1 are moved to array2. On even passes (log10(placeValue) % 2 ==
     * 0), numbers from array2 are moved to array1.
     *
     * The purpose of using this method is to reduce the replication of array,
    ...
    private static void sortingPass(int[] array, int placeValue) {
        ...
        /**
         * Stores the current digit of a number at the given place value. For
         * example: if placeValue == 10, we're sorting by the tens place. If the
         * number is 30, then digit = 3.
         */
        int digit;

        /**
         * First if statement. On the first pass where placeValue == 1, sorting based
         * on the current place value & moves numbers from input array to array1.
         */
        ...
        if (placeValue == 1) {
            ...
            /**
             * One extra comparison (i < array.length).
             * One accessing member variable (array.length).
             */
            operationCount += 2;
            resetCharRowCounts();
         ...
         * Executes during odd passes (when placeValue is 10, 1000, 100000,
         * etc.) 
         * Sort based on the current placeValue which moves numbers from array1
         * to array2.
         */
            ...
            for (int i = 0; i < array1.length; i++) { // Outer loop, loop through 10 digit rows of array1.
                ...
                for (int j = 0; j < array1[i].length; j++) { // Inner loop, loop through numbers contains in the particular digit row.
                    ...
                    // One comparison (currentNumber == null).
                    operationCount += 1;
                    if (currentNumber == null) {
                        break; // If the current number is null, means there's no other more numbers left in this digit rows, we break the inner loop.
                    }
                    digit = (currentNumber / placeValue) % 10; // Get the current numbers digit based on the placeValue.
                    /**
                     * One division (/).
                     ...
                     * - digitRowCounts[digit] one assignment (store number into digit row).
                     */
                    operationCount += 4;
                    digitRowCounts[digit]++; // Increment the counts of element in that row.
                ...
                /**
                 * One extra comparison (j < array1[i].length).
                 * Acessing member variable (array1[i].length).
                 * One array indexing (array1[i]).
                 */
                operationCount += 3;
            }
            /**
             * One extra comparison (i < array1.length).
             * One accessing member variable (array1.length).
             */
            operationCount += 2;
            clearArray(array1);
         ...
         * Executes during even passes (when placeValue is 100, 10000, 1000000,
         * etc.) 
         * Sort based on the current placeValue Moves numbers from array2
         * to array1.
         */
        else {
            /**
             * If the code reaches here, it means the condition in the previous
             * 'else if' was false. Increase operations involved in the
             * evaluation of the 'else if' condition:
             * One method call (log10()) 
             * One modulo (%) 
             * One comparison.
             */
            ...
            for (int i = 0; i < array2.length; i++) { // Outer loop, loop through 10 digit rows of array2.
                ...
                /**
                 * One extra comparison (j < array2[i].length)
                 * One accessing member variable (array1[i].length).
                 * One array indexing (array1[i]).
                 */
                operationCount += 3;
            ...
            /**
             * One extra comparison (i < array2.length).
             * One accessing member variable (array2.length).
             */
            operationCount += 2;
            clearArray(array2);
    ...
    /**
     * Displays the current state of the digit rows during the sorting pass
     * based on the given placeValue. The method determines which bucket array
     * (array1 or array2) to display based on whether the place value
     * corresponds to an odd or even digit pass.
     *
     * @param placeValue the current place value (1, 10, 100, etc.) based on
     * which sorting pass is happening.
     *
     * - Chooses the active 2D array (either array1 or array2) depending on
     * whether the current place value is odd or even. - Iterates over each
     * digit row (0-9) and prints the numbers stored in it, stopping at null
     * (empty spots, means there is no more numbers left behind).
     */
    ...
    /**
     * Method used to reorder numbers from the 2D array into a 1D array after
     * the final sorting pass. It scans the final active sorting array (array1
     * or array2 depending on maxStringLength) and transfers all non-null values
     * into a one-dimensional SortedList array.
     */
    ...
    public static void Sort(int[] numbers) {
        ...
        operationCount += 1; // One method call (initializeArray()).
        ...
        /**
         * One method call (findLargestValue()).
         * One assignment (largestValue = findLargestValue(numbers)).
         */
        operationCount += 2; 

        // Calculate how many digit places (maxStringLength) the sorting needs to process.
        calculatemaxStringLength(largestValue);
        operationCount += 1; // One method call (calculatemaxStringLength()).
        ...
        for (int placeValue = 1; largestValue / placeValue > 0; placeValue *= 10) {
             ...
             * One division (largestValue / placeValue).
             * One comparison (largestValue / placeValue > 0).
             */
            operationCount += 4;
    ...
    /**
     * Methods to determine the length of largest digit given the largest value
     * in the input numbers array (e.g.: 3141 digit length is 4).
     *
    ...
    public static void main(String[] args) {
        ...
        int[] numbers = {275, 87, 426, 61, 409, 170, 677, 503, 8, 312, 55, 789, 203, 17, 98, 471, 120, 3, 654, 222, 76, 41, 997, 29, 246, 19, 531, 84, 301, 67, 911, 38, 143, 27, 600, 72, 90, 451, 265, 5, 356, 13, 199, 63, 288, 81, 7, 740, 266, 111, 204, 66, 882, 140, 59, 391, 24, 217, 452, 100, 12, 621, 303, 58, 33, 875, 186, 493, 77, 49, 149, 268, 101, 315, 36, 879, 115, 44, 20, 528};
