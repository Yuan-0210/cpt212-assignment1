public class Algorithm2 {
    ...
    public static int findLargestDigitLength(String[] array) {
        int max = 0;
        operationCounts+=1; // 1 assign
        operationCounts+=1; // 1 assign
        for (int i = 0; i < array.length; i++) {
            ...
            operationCounts+=2; // 1 compare, 1 call
            if (str.length() > max) {
                max = str.length();
                operationCounts+=2; // 1 assign, 1 call
            }
        }
        operationCounts+=1; // 1 return
        return max;
    ...
    private static String[] sort(String[] array, int exp) {
        ...
        operationCounts+=2; // 1 arithmetic, 1 compare
        if (exp % 2 == 0) {
        ...
        */
        ...
        for (int i = 0; i < array.length; i++) {
            operationCounts+=3; // 1 assign, 1 compare, 1 op
            int index;
            operationCounts+=3; // 1 compare, 1 lookup, 1 call
            if (exp < array[i].length()) {
                char ch = array[i].charAt(exp);
                operationCounts+=3; // 1 assign, 1 lookup, 1 call
                index = ch - 'a' + 1;
                operationCounts+=3; // 1 assign, 2 arithmetic
            } else {
                index = 0; // "missing character"
                operationCounts+=1; // 1 assign
            }
            count[index]++;
            operationCounts+=3; // 1 lookup, 1 op, 1 assign
        }
        ...
        */
        ...
        for (int i = 1; i < 27; i++) {
            operationCounts+=3; // 1 assign, 1 compare, 1 op
            count[i] += count[i - 1];
            operationCounts+=5; // 2 lookup, 2 arithmetic, 1 assign
        }

        // Build output array (iterate in reverse for stability)
        operationCounts+=1; // 1 assign
        for (int i = array.length - 1; i >= 0; i--) {
            operationCounts+=4; // 1 assign, 1 arithmetic, 1 compare, 1 op
            int index;
            operationCounts+=3; // 1 compare, 1 lookup, 1 call
            if (exp < array[i].length()){
                char ch = array[i].charAt(exp);
            ...
            operationCounts+=5; // 1 arithmetic, 3 lookup, 1 assign
            count[index]--;
            operationCounts+=3; // 1 lookup, 1 op, 1 assign
        }
        operationCounts+=1; // 1 return
    ...
    public static void main(String[] args) {
       ...
       ;
        ...
        int maxLen = findLargestDigitLength(words);
        operationCounts+=2; // 1 assign, 1 call
        ...
        for (int exp = maxLen - 1; exp >= 0; exp--) {
            ...
            for (int i = 1; i < 27; i++) {
                ...
                operationCounts+=5; // 1 lookup, 1 assign , 3 arithmetic
            }
            operationCounts+=1; // 1 assign
            for (int i = 0; i < words.length; i++) {
                operationCounts+=3; // 1 assign, 1 compare, 1 op
                ...
                if (exp < words[i].length()) {
                    ...
                    operationCounts+=3; // 1 assign, 1 lookup, 1 call
                    index = ch - 'a' + 1;
                    operationCounts+=3; // 2 arithmetic, 1 assign
                } else {
                    index = 0;
                    operationCounts+=1; // 1 assign 
                }
        ...
        for (String word : words) {
            operationCounts+=2; // 1 op, 1 assign
            System.out.print(word + " ");
            operationCounts+=1; // 1 arithmetic
        }
